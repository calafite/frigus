...
step_pay_bounty(W, Id, NW, Evts) :-
    ...
    room(A, RId), world:node(W, RId, N), member(town_hall, N.props),
    bounty(A, B), B > 0, inv(A, Inv), inv_rem(Inv, gold, B, NInv),
    world:update(W, A.put(inv, NInv).put(bounty, 0).put(fac, citizen), NW),
    Evts = [bounty_paid(Id, B)].

step_jailbreak(W, Id, NW, Evts) :-
    world:entity(W, Id, A), alive(A), status:can_act(A),
    room(A, RId), world:node(W, RId, N), member(prison, N.props),
    ...
    ( Score >= 20 ->
        ...
        get_dict(Dir, N.exits, NRId), entity:room(A, NRId, A1),
...
step_bribe_guard(W, Id, GuardId, NW, Evts) :-
    ...
    bounty(A, B), B > 0, B < 2000, BribeAmt is floor(B * 1.5),
