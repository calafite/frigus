:- module(combat, [do_kill/3, do_cast/4, is_town_npc/1]).
...
apply_damage(SrcId, SrcEnt, Tgt, Dmg, Evts) :-
    ...
    ( entity:is_alive(NTgt) ->
        ( (is_dict(NTgt, mob), is_dict(SrcEnt, plyr)) ->
            entity:add_threat(NTgt, SrcId, Dmg, ThreatTgt)
        ;
            ThreatTgt = NTgt
        ),
        world:put_entity(ThreatTgt),
        ( (is_dict(ThreatTgt, mob), is_dict(SrcEnt, plyr)) ->
            ( is_town_npc(ThreatTgt) ->
                town_brawl_retaliate(ThreatTgt, SrcEnt, RetalEvts)
            ;
                mob_retaliate(ThreatTgt, SrcEnt, RetalEvts)
            )
        ;
            RetalEvts = []
        ),
        append([hit(SrcId, TgtId, Dmg, CurHp, MaxHp)], RetalEvts, Evts)
    ;
        handle_death(SrcEnt, NTgt, DeathEvts),
        Evts = [hit(SrcId, TgtId, Dmg, 0, MaxHp), dead(TgtId) | DeathEvts]
    ).
...
get_mob_base_damage(Mob, BaseDmg) :-
    ( get_dict(equip, Mob, Eq), is_dict(Eq), get_d:- module(combat, [do_kill/3, do_cast/4]).

    :- use_module('../core/world').
    :- use_module('../core/entity', [is_alive/1, get_stat/3, mod_hp/3]).
    :- use_module('../config/combat').
    :- use_module('../config/spawn').
    :- use_module('prog').
    :- use_module('loot').
    ...
    apply_damage(SrcId, SrcEnt, Tgt, Dmg, Evts) :-
        ...
        ( entity:is_alive(NTgt) ->
            ...
            ( (is_dict(NTgt, mob), is_dict(SrcEnt, plyr)) ->
                ( is_town_npc(NTgt) ->
                    town_brawl_retaliate(NTgt, SrcEnt, RetalEvts)
                ;
                    mob_retaliate(NTgt, SrcEnt, RetalEvts)
                )
            ;
                RetalEvts = []
            ),
    ...
    brawl_attack_all([], _, []).
    brawl_attack_all([Mob|Rest], Player, Evts) :-
        mob_retaliate(Mob, Player, SingleEvts),
        brawl_attack_all(Rest, Player, RestEvts),
        append(SingleEvts, RestEvts, Evts).
    ...
    handle_death(_SrcEnt, DeadTgt, [respawned(TgtId, square)]) :-
    ...
    handle_death(SrcEnt, DeadMob, Evts) :-
