...
chat_tgts(W, party, Id, Tgts) :- module(social, [
    step_chat/6, step_party/5, step_guild/5,
    party_reward/7, soc/2, soc/3, db_social/1
]).
...
chat_tgts(W, party, Id, Tgts) :-
    ...
    soc(W, S), get_dict(parties, S, Ps), get_dict(PId, Ps, P),
    get_dict(members, P, Tgts), !.
chat_tgts(W, guild, Id, Tgts) :-
    world:entity(W, Id, A), get_dict(guild, A, GId), GId \== none,
    soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G),
    get_dict(members, G, Tgts), !.
...
step_party(W, Id, create, NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(party, A, CurP), CurP \== none ->
        NW = W, Evts = [already_in_party(Id, CurP)]
    ;
        random_between(10000, 99999, R), atomic_list_concat([party_, R], PId),
        soc(W, S), ( get_dict(parties, S, Ps) -> true ; Ps = dict{} ),
        P = dict{id: PId, ldr: Id, members: [Id]},
        NPs = Ps.put(PId, P), soc(W, S.put(parties, NPs), W1),
        world:update(W1, A.put(party, PId), NW),
        Evts = [party_created(Id, PId)]
    ).

step_party(W, Id, invite(Tgt), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(party, A, PId), PId \== none,
      soc(W, S), get_dict(parties, S, Ps), get_dict(PId, Ps, P), P.ldr == Id
    ->
        NW = W, Evts = [party_inv(Id, Tgt, PId)]
    ;
        NW = W, Evts = [not_party_leader(Id)]
    ).

step_party(W, Id, join(PId), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(party, A, CurP), CurP \== none ->
        NW = W, Evts = [already_in_party(Id, CurP)]
    ; soc(W, S), get_dict(parties, S, Ps), get_dict(PId, Ps, P) ->
        NP = P.put(members, [Id|P.members]),
        NPs = Ps.put(PId, NP), soc(W, S.put(parties, NPs), W1),
        world:update(W1, A.put(party, PId), NW),
        Evts = [party_joined(Id, PId)]
    ;
        NW = W, Evts = [party_not_found(Id, PId)]
    ).

step_party(W, Id, leave, NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(party, A, PId), PId \== none,
      soc(W, S), get_dict(parties, S, Ps), get_dict(PId, Ps, P)
    ->
        select(Id, P.members, NMs),
        ( NMs == [] -> del_dict(PId, Ps, _, NPs)
        ; P.ldr == Id -> NMs = [NLdr|_], NPs = Ps.put(PId, P.put(ldr, NLdr).put(members, NMs))
        ; NPs = Ps.put(PId, P.put(members, NMs)) ),
        soc(W, S.put(parties, NPs), W1),
        world:update(W1, A.put(party, none), NW),
        Evts = [party_left(Id)]
    ;
        NW = W, Evts = [not_in_party(Id)]
    ).

step_party(W, Id, kick(Tgt), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(party, A, PId), PId \== none,
      soc(W, S), get_dict(parties, S, Ps), get_dict(PId, Ps, P), P.ldr == Id,
    ...
    ->
        NPs = Ps.put(PId, P.put(members, NMs)),
        soc(W, S.put(parties, NPs), W1),
        ( world:entity(W1, Tgt, T) -> world:update(W1, T.put(party, none), NW) ; NW = W1 ),
        Evts = [party_kicked(Tgt)]
    ;
        NW = W, Evts = [cannot_kick_party(Id, Tgt)]
    ).
...
step_guild(W, Id, create(Name), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, CurG), CurG \== none ->
        NW = W, Evts = [already_in_guild(Id, CurG)]
    ;
        random_between(10000, 99999, R), atomic_list_concat([guild_, R], GId),
        soc(W, S), ( get_dict(guilds, S, Gs) -> true ; Gs = dict{} ),
        G = dict{id: GId, name: Name, master: Id, officers: [], members: [Id], stash: []},
        NGs = Gs.put(GId, G), soc(W, S.put(guilds, NGs), W1),
        world:update(W1, A.put(guild, GId), NW),
        Evts = [guild_created(Id, GId)]
    ).

step_guild(W, Id, invite(Tgt), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, GId), GId \== none,
      soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G),
      ( G.master == Id ; member(Id, G.officers) )
    ->
        NW = W, Evts = [guild_inv(Id, Tgt, GId)]
    ;
        NW = W, Evts = [not_guild_officer(Id)]
    ).

step_guild(W, Id, join(GId), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, CurG), CurG \== none ->
        NW = W, Evts = [already_in_guild(Id, CurG)]
    ; soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G) ->
        NG = G.put(members, [Id|G.members]),
        NGs = Gs.put(GId, NG), soc(W, S.put(guilds, NGs), W1),
        world:update(W1, A.put(guild, GId), NW),
        Evts = [guild_joined(Id, GId)]
    ;
        NW = W, Evts = [guild_not_found(Id, GId)]
    ).

step_guild(W, Id, leave, NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, GId), GId \== none,
      soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G), G.master \== Id
    ->
        select(Id, G.members, NMs),
        ( select(Id, G.officers, NOs) -> true ; NOs = G.officers ),
        NGs = Gs.put(GId, G.put(members, NMs).put(officers, NOs)),
        soc(W, S.put(guilds, NGs), W1),
        world:update(W1, A.put(guild, none), NW),
        Evts = [guild_left(Id)]
    ;
        NW = W, Evts = [cannot_leave_guild(Id)]
    ).

step_guild(W, Id, kick(Tgt), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, GId), GId \== none,
      soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G),
    ...
    ->
        ...
        NGs = Gs.put(GId, G.put(members, NMs).put(officers, NOs)),
        soc(W, S.put(guilds, NGs), W1),
        ( world:entity(W1, Tgt, T) -> world:update(W1, T.put(guild, none), NW) ; NW = W1 ),
        Evts = [guild_kicked(Tgt)]
    ;
        NW = W, Evts = [cannot_kick_guild(Id, Tgt)]
    ).

step_guild(W, Id, promote(Tgt), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, GId), GId \== none,
      soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G), G.master == Id,
      member(Tgt, G.members), \+ member(Tgt, G.officers)
    ->
        NGs = Gs.put(GId, G.put(officers, [Tgt|G.officers])),
        soc(W, S.put(guilds, NGs), NW),
        Evts = [guild_promoted(Tgt)]
    ;
        NW = W, Evts = [cannot_promote(Id, Tgt)]
    ).

step_guild(W, Id, stash_put(Tag, Q), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, GId), GId \== none,
      inv(A, Inv), inv_rem(Inv, Tag, Q, NInv)
    ->
        A1 = A.put(inv, NInv),
        soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G),
        ( get_dict(stash, G, Stash) -> true ; Stash = [] ),
        inv_add(Stash, Tag, Q, NStash),
        NGs = Gs.put(GId, G.put(stash, NStash)),
        soc(W, S.put(guilds, NGs), W1),
        world:update(W1, A1, NW),
        Evts = [guild_stash_put(Id, Tag, Q)]
    ;
        NW = W, Evts = [cannot_stash_put(Id, Tag)]
    ).

step_guild(W, Id, stash_take(Tag, Q), NW, Evts) :-
    world:entity(W, Id, A),
    ( get_dict(guild, A, GId), GId \== none,
      soc(W, S), get_dict(guilds, S, Gs), get_dict(GId, Gs, G),
      get_dict(stash, G, Stash), inv_rem(Stash, Tag, Q, NStash)
    ->
        inv(A, Inv), inv_add(Inv, Tag, Q, NInv), A1 = A.put(inv, NInv),
        NGs = Gs.put(GId, G.put(stash, NStash)),
        soc(W, S.put(guilds, NGs), W1),
        world:update(W1, A1, NW),
        Evts = [guild_stash_take(Id, Tag, Q)]
    ;
        NW = W, Evts = [cannot_stash_take(Id, Tag)]
    ).
    world:entity(W, Id, A), get_dict(party, A, PId), PId \== none,
